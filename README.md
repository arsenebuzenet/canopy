...
**2. The human / dashboard management surface — CLI `--json`.** PR triage, review-comment classification, bot rollups, ship, historian, resume briefs, conflict detection, Linear/GitHub reads, against GitHub or Bitbucket Cloud remotes (detected per repo) — the management work — is *not* on the agent surface. It lives in `canopy/management/` and is reached by the human (or the dashboard/GUI) via `canopy <cmd> --json`. Same JSON shape across CLI, MCP, and any GUI.
