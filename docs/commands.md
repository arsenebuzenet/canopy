...
| `canopy ship [--feature <f>] [--repo <r,...>] [--draft] [--reviewers <h,...>] [--base <branch>] [--dry-run]` | Open or update one PR per repo in the canonical (or named) feature, with cross-repo body links; the platform (GitHub or Bitbucket Cloud) is detected from each repo's remote. `--draft` opens as drafts (initial open only); `--reviewers` requests review — GitHub handles on GitHub; nicknames, display names or `{uuid}`s on Bitbucket Cloud (ambiguous names are rejected); `--base` overrides the base branch (default: each repo's `default_branch`); `--dry-run` enumerates without pushing or opening PRs. |
...
| ⚡ `canopy worktree-bootstrap <feature> [--force] [--step env\|deps\|ide] [--deps] [--interactive]` **(agent tool)** | Bootstrap a feature's worktrees by hand: env-file copy (per-repo `env_files` into each worktree, `[workspace] env_files` into the slot dir), `install_cmd`, IDE workspace gen, and the per-clone hook step. `--step` restricts to one step; `--deps` runs deps only (retry a `failed`/`installing` slot, or the target of the background install `context` points at); `--interactive` runs the deps install in the foreground to stream output / handle prompts; `--force` overwrites existing env files and bypasses the lockfile-unchanged short-circuit. |
