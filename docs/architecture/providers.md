...
- **Code-review platforms** (GitHub, Bitbucket Cloud) — *implemented in
  4.0.0-rc5, deliberately without the provider registry*:
  [`integrations/review.py`](../../src/canopy/integrations/review.py)
  dispatches on the repo's remote host (`platforms.parse_remote`) to
  `integrations/github.py` or `integrations/bitbucket.py`. Listed here only
  because a GitLab / Bitbucket Data Center backend would be the point at
  which a `ReviewPlatformProvider` protocol might earn its place.
